...
public class RepositoryMirrorsPlugin implements Plugin<Gradle> {

    private List<MirrorMetadata> mirrors = new ArrayList<>();

    @Override
    public void apply(@NonNull Gradle gradle) {

        ServiceLoader.load(MirrorMetadataProvider.class).forEach(provider -> {
    ...
    private void applyMirror(MavenArtifactRepository repository) {
        ...
        for (MirrorMetadata mirror : mirrors) {
